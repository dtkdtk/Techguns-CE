...
public class GenericGun extends GenericItem implements IGenericGun, IItemTGRenderer, ICamoChangeable {
            ...
            if (this.projectile_selector.ammoType.hasMultipleVariants()) {
                ...
                if (this.projectile_selector == ROCKET_PROJECTILES || this.projectile_selector == GUIDED_MISSILE_PROJECTILES) {
                    ...
                    for (int i = 0; i < ammoCount; i++) {
                        int amount = InventoryUtil.addAmmoToPlayerInventory(player, new ItemStack(this.getAmmoType().getBullet(currentVariant)[0].getItem(), 1, this.getAmmoType().getBullet(currentVariant)[0].getItemDamage()));
                        if (amount > 0 && !world.isRemote) {
                            player.world.spawnEntity(new EntityItem(player.world, player.posX, player.posY, player.posZ, new ItemStack(this.ammoType.getBullet(currentVariant)[0].getItem(), amount, this.ammoType.getBullet(currentVariant)[0].getItemDamage())));
                        }
                    ...
                    for (int i = 0; i < ammos; i++) {
                        if (!this.ammoType.getEmptyMag()[i].isEmpty()) {
                            int amount = InventoryUtil.addAmmoToPlayerInventory(player, new ItemStack(this.getAmmoType().getAmmo(currentVariant)[i].getItem(), 1, this.getAmmoType().getAmmo(currentVariant)[i].getItemDamage()));
                            if (amount > 0 && !world.isRemote) {
                                player.world.spawnEntity(new EntityItem(player.world, player.posX, player.posY, player.posZ, new ItemStack(this.ammoType.getAmmo(currentVariant)[i].getItem(), amount, this.ammoType.getAmmo(currentVariant)[i].getItemDamage())));
                            }
                        }
                    }
                } else if (oldAmmo > 0) {
                    ...
                    for (int i = 0; i < ammos; i++) {
                        if (!this.ammoType.getEmptyMag()[i].isEmpty()) {
                            int amount = InventoryUtil.addAmmoToPlayerInventory(player, new ItemStack(this.ammoType.getEmptyMag()[i].getItem(), 1, this.ammoType.getEmptyMag()[i].getItemDamage()));
                            if (amount > 0 && !world.isRemote) {
                                player.world.spawnEntity(new EntityItem(player.world, player.posX, player.posY, player.posZ, new ItemStack(this.ammoType.getEmptyMag()[i].getItem(), amount, this.ammoType.getEmptyMag()[i].getItemDamage())));

                                int bulletsBack = (int) Math.floor(oldAmmo / this.ammoType.getShotsPerBullet(clipsize, oldAmmo));
                                if (bulletsBack > 0) {
                                    int amount2 = InventoryUtil.addAmmoToPlayerInventory(player, new ItemStack(this.ammoType.getBullet(currentVariant)[i].getItem(), bulletsBack, this.ammoType.getBullet(currentVariant)[i].getItemDamage()));
                                    if (amount2 > 0) {
                                        player.world.spawnEntity(new EntityItem(player.world, player.posX, player.posY, player.posZ, new ItemStack(this.ammoType.getBullet(currentVariant)[i].getItem(), amount2, this.ammoType.getBullet(currentVariant)[i].getItemDamage())));
                                    }
                                }
                            }
                        }
    ...
    public boolean onLeftClickEntity(@NotNull ItemStack stack, @NotNull EntityPlayer player, @NotNull Entity targetEntity) {
        ...
        if (!this.shootWithLeftClick) {
             ...
             */
            if (targetEntity.canBeAttackedWithItem() && GenericProjectile.BULLET_TARGETS.test(targetEntity)) {
    ...
    protected void consumeAmmoOnMeleeHit(EntityLivingBase elb, ItemStack stack) {
        if (elb instanceof EntityPlayer ply) {
    ...
    protected void doSweepAttackEffect(EntityPlayer player) {
        if (!player.world.isRemote) {
            ...
            this.spawnSweepParticle(player.world, x, y, z, d0, d1);
            ...
            if (!this.isFullyLoaded(stack)) {
                ...
                for (int i = 0; i < amount; i++) {
                    ...
                    if (bulletsBack > 0) {
                        ...
                        items.add(new ItemStack(bullet.getItem(), bulletsBack, bullet.getItemDamage()));
                    }
                    if (!this.ammoType.getEmptyMag()[i].isEmpty()) {
                        items.add(new ItemStack(this.ammoType.getEmptyMag()[i].getItem(), 1, this.ammoType.getEmptyMag()[i].getItemDamage()));
                    }
                }
            } else {
                int amount = this.ammoType.getEmptyMag().length;
                for (int i = 0; i < amount; i++) {
                    items.add(new ItemStack(this.ammoType.getAmmo(this.getCurrentAmmoVariant(stack))[i].getItem(), 1,
                            this.ammoType.getAmmo(this.getCurrentAmmoVariant(stack))[i].getItemDamage()));
                }
            }
        }
            ...
            if (InventoryUtil.consumeAmmoPlayer(player, this.getReloadItem(item))) {
                ...
                for (int i = 0; i < ammos; i++) {
                    if (!this.ammoType.getEmptyMag()[i].isEmpty()) {
                        ...
                        int bulletsBack = (int) Math.floor(oldAmmo / this.ammoType.getShotsPerBullet(clipsize, oldAmmo));
                        if (bulletsBack > 0) {
                            int amount2 = InventoryUtil.addAmmoToPlayerInventory(player, new ItemStack(this.ammoType.getBullet(this.getCurrentAmmoVariant(item))[i].getItem(), bulletsBack, this.ammoType.getBullet(this.getCurrentAmmoVariant(item))[i].getItemDamage()));
                            if (amount2 > 0 && !world.isRemote) {
                                player.world.spawnEntity(new EntityItem(player.world, player.posX, player.posY, player.posZ, new ItemStack(this.ammoType.getBullet(this.getCurrentAmmoVariant(item))[i].getItem(), amount2,
                                        this.ammoType.getBullet(this.getCurrentAmmoVariant(item))[i].getItemDamage())));
                            }
                        }
